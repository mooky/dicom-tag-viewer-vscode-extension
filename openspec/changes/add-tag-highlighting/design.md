...
**Decision:** A dedicated gutter column (separate from the existing 22px note gutter) renders one solid bar per highlight active on that row, ordered left-to-right in creation order (oldest leftmost). The most-recently-created active highlight on a row also contributes a faint full-row background wash (~5% alpha) for extra legibility.
